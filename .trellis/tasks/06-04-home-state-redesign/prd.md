...
- Follow-up requirement on 2026-06-05: logged-in mobile home content should be reduced to only the search box and home bookmarks, while retaining a lightweight top-right user/account entry.
...
- Logged-in mobile home must show only search and enabled home bookmarks as content; it must retain a top-right user/account entry and must not show featured shortcuts, category panels, overview, todos, notes, or other dashboard widgets.
...
- [ ] Logged-in mobile view contains only search and enabled home bookmarks after loading succeeds, plus a top-right user/account entry.
